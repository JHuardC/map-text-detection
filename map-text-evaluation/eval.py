...
Script copied from: https://github.com/icdar-maptext/evaluation
...
from typing import Final, Union, Tuple, Any, Optional, Callable
...
LOCAL_DIR: Final[Path] = Path(environ["LOCAL_DIR"])
...
parser.add_argument('--gt', type=LOCAL_DIR.joinpath, required=True,
                    help="Path to the ground truth JSON file, relative to LOCAL_DIR environment variable.")
parser.add_argument('--pred', type=LOCAL_DIR.joinpath, required=True,
                    help="Path to the predictions JSON file, relative to LOCAL_DIR environment variable.")
parser.add_argument('--output', type=LOCAL_DIR.joinpath, required=False, default=None,
                    help="Path to the JSON file containing results, relative to LOCAL_DIR environment variable.")
...
parser.add_argument('--parallel', type=str, required=False, default='none',
                    choices=['none', 'pool'],
                    help="Process evaluation in parallel using multiprocessing")
    ...
    for image_groups in data.values():
        for group in image_groups:
            for word in group['words']:
                ...
                word['geometry'] = shapely.geometry.Polygon(points_closed).buffer(0)
